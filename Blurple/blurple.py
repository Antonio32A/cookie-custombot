...
allowed_users_list = set([int(i) for i in os.getenv('ALLOWED_USERS').split(',')])
approved_channels = set([int(i) for i in os.getenv('ALLOWED_CHANNELS').split(',')])
...
async def on_connect():
    ...
    print(bot.user.id)
    print('------')
    activity = discord.Game(name="Try ?help or b.help")
